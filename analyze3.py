    ...
    for weight_type in weight_types:
        ...
        for subject_id in subject_ids:
            ...
            for day in days:
                ...
                # '가격'이 0이 아닌 경우에만 합산
                cnt += len(subject_df[subject_df['가격'] != 0])
                expense += subject_df[subject_df['가격'] != 0]['가격'].sum()
            
            # 일주일 평균 지출 계산
            if cnt == 0:
                continue
            average_expense = expense / cnt
            results[weight_type].append(average_expense)
...
plt.title('체중 유형별 일 평균 식비', fontsize=16, fontweight='bold')
plt.xlabel('체중유형', fontsize=12, labelpad = 15)
plt.ylabel('일 평균 식비 (단위: 원)', fontsize=12)
# plt.show()
plt.savefig('일평균식비.png')
